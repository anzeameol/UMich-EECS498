...
def cifar10(num_train=None, num_test=None, x_dtype=torch.float32):
  ...
  - num_test: [Optional] How many samples to keep from the test.py set.
    If not provided, then keep the entire test.py set.
  - x_dtype: [Optional] Data type of the input image
...
def preprocess_cifar10(
  ...
  - 'X_test': `dtype` tensor of shape (N_test, D) giving test.py images
  - 'y_train': int64 tensor of shape (N_train,) giving training labels
  - 'y_val': int64 tensor of shape (N_val,) giving val labels
  - 'y_test': int64 tensor of shape (N_test,) giving test.py labels

  N_train, N_val, and N_test are the number of examples in the train, val, and
  test.py sets respectively. The precise values of N_train and N_val are determined
  ...
  # Note: It should not be taken from the test.py set
