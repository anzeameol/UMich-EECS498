...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    Returns:
    If y is None, then run a test.py-time forward pass of the model and return:
    ...
    # If y is None then we are in test.py mode so just return scores
    if y is None:
      return scores
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    # (train / test.py). You can pass the same dropout_param to each dropout layer.
    self.dropout_param = {}
    if self.use_dropout:
  ...
  def loss(self, X, y=None):
    ...
    """
    X = X.to(self.dtype)
    mode = 'test.py' if y is None else 'train'

    # Set train/test.py mode for batchnorm params and dropout param since they
...
class Dropout(object):
  ...
  def forward(x, dropout_param):
    ...
    - dropout_param: A dictionary with the following keys:
      ...
      - mode: 'test.py' or 'train'. If the mode is train, then perform dropout;
      if the mode is test.py, then just return the input.
    ...
    - cache: tuple (dropout_param, mask). In training mode, mask is the dropout
      mask that was used to multiply the input; in test.py mode, mask is None.
    ...
    if mode == 'train':
      ...
      ###########################################################################
      # TODO: Implement the test.py phase forward pass for inverted dropout.       #
      ###########################################################################
      # Replace "pass" statement with your code
      out = x
      ###########################################################################
