...
def compute_distances_two_loops(x_train, x_test):
  ...
  set and each element of the test.py set. Images should be flattened and treated
  as vectors.
  ...
  the inputs will have shape (num_{train, test.py}, D); it should alse be able to
  compute nearest neighbors between images, where the inputs will have shape
  (num_{train, test.py}, C, H, W). More generally, the inputs will have shape
  (num_{train, test.py}, D1, D2, ..., Dn); you should flatten each element
  ...
  - dists: Torch tensor of shape (num_train, num_test) where dists[i, j] is the
    squared Euclidean distance between the ith training point and the jth test.py
  ...
  """
    ...
    # training data and the test.py data.                                           #
    #                                                                            #
...
def predict_labels(dists, y_train, k=1):
    """
  Given distances between all pairs of training and test.py samples, predict a
  label for each test.py sample by taking a **majority vote** among its k nearest
  neighbors in the training set.

  In the event of a tie, this function **should** return the smallest label. For
  example, if k=5 and the 5 nearest neighbors to a test.py example have labels
...
s
  ...
  - y_pred: A torch int64 tensor of shape (num_test,) giving predicted labels
    for the test.py data, where y_pred[j] is the predicted label for the jth test.py
    example. Each label should be an integer in the range [0, num_classes - 1].
  """
    num_train, num_test = dists.shape
    y_pred = torch.zeros(num_test, dtype=torch.int64)
    ##############################################################################
    # TODO: Implement this function. You may use an explicit loop over the test.py  #
...
class KnnClassifier:
    ...
    Inputs:
    - x_test: Torch tensor of shape (num_test, C, H, W) giving test.py samples
    - k: The number of neighbors to use for predictions

    Returns:
    - y_test_pred: Torch tensor of shape (num_test,) giving predicted labels
      for the test.py samples.
    """
        y_test_pred = None
        ###########################################################################
    ...
    def check_accuracy(self, x_test, y_test, k=1, quiet=False):
        """
    Utility method for checking the accuracy of this classifier on test.py data.
    Returns the accuracy of the classifier on the test.py data, and also prints a
    message giving the accuracy.

    Inputs:
    - x_test: Torch tensor of shape (num_test, C, H, W) giving test.py samples
    - y_test: int64 torch tensor of shape (num_test,) giving test.py labels
    - k: The number of neighbors to use for prediction
    - quiet: If True, don't print a message.

    Returns:
    - accuracy: Accuracy of this classifier on the test.py data, as a percent.
