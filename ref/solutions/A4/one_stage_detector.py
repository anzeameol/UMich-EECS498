...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(self, num_classes: int, in_channels: int, stem_channels: List[int]):
        ...
        for c in stem_channels:
            conv = nn.Conv2d(in_c, c, 3, padding=1)
        ...
        self.pred_cls = nn.Conv2d(c, num_classes, 3, padding=1)  # Class prediction conv
        self.pred_box = nn.Conv2d(c, 4, 3, padding=1)  # Box regression conv
        self.pred_ctr = nn.Conv2d(c, 1, 3, padding=1)  # Centerness conv
        self.pred_cls.bias.data.fill_(0.0)
        nn.init.normal_(self.pred_cls.weight, 0, 0.01)
        self.pred_box.bias.data.fill_(0.0)
        nn.init.normal_(self.pred_box.weight, 0, 0.01)
        self.pred_ctr.bias.data.fill_(0.0)
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for key in feats_per_fpn_level.keys():
            ...
            boxreg_deltas[key] = (
                boxreg_deltas[key].reshape(B, -1, H * W).transpose(1, 2)
            )
            centerness_logits[key] = self.pred_ctr(x_box)
            centerness_logits[key] = (
                centerness_logits[key].reshape(B, -1, H * W).transpose(1, 2)
            )
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def fcos_get_deltas_from_locations(
    ...
    deltas[:, 2] = -locations[:, 0] + gt_boxes[:, 2]
    deltas[:, 3] = -locations[:, 1] + gt_boxes[:, 3]
...
def fcos_apply_deltas_to_locations(
    ...
    output_boxes[deltas < 0] = 0
    output_boxes = output_boxes * stride
    output_boxes[:, 0] = locations[:, 0] - output_boxes[:, 0]
    output_boxes[:, 1] = locations[:, 1] - output_boxes[:, 1]
    output_boxes[:, 2] = locations[:, 0] + output_boxes[:, 2]
    output_boxes[:, 3] = locations[:, 1] + output_boxes[:, 3]
    output_boxes[deltas[:, 0] == -1] = locations.repeat(1, 2)[deltas[:, 0] == -1]
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    centerness = None
    # Replace "pass" statement with your code
    val1 = torch.min(deltas[:, 0], deltas[:, 2])
    val2 = torch.min(deltas[:, 1], deltas[:, 3])
    val3 = torch.max(deltas[:, 0], deltas[:, 2])
    val4 = torch.max(deltas[:, 1], deltas[:, 3])
    centerness = torch.sqrt(val1 * val2 / val3 / val4)
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        pred_cls_logits, pred_boxreg_deltas, pred_ctr_logits = self.pred_net(feats)
        ...
        matched_gt_boxes = [
            fcos_match_locations_to_gt(
                locations_per_fpn_level, self.backbone.fpn_strides, box
            )
            for box in gt_boxes
        ]
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            level_pred_scores = level_pred_scores[
                torch.arange(level_pred_scores.shape[0]), level_pred_classes
            ]

            # Step 2:
            # Replace "pass" statement with your code
            level_pred_classes = level_pred_classes[
                level_pred_scores > test_score_thresh
            ]
