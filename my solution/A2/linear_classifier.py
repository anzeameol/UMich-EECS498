...
def svm_loss_vectorized(
    ...
    #############################################################################
    # Replace "pass" statement with your code
    dW += X.t().mm(take.type_as(X))
    t2 = torch.zeros((num_train, num_classes), device=X.device, dtype=X.dtype)
    t2[range(num_train),y] = 1
    dW -= (X * take.sum(dim=1,keepdim=True)).t().mm(t2)
    dW = dW / num_train + 2 * reg * W
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def sample_batch(
    ...
    #########################################################################
    # Replace "pass" statement with your code
    rand_num = torch.randint(high=len(X), low=0, size=(batch_size,))
    X_batch = X[rand_num]
    y_batch = y[rand_num]
    #########################################################################
    #                       END OF YOUR CODE                                #
...
def svm_get_search_params():
    ...
    regularization_strengths = [0.1, 0.5, 1, 5]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def test_one_param_set(
    ...
    train_acc = (y_train_pred == data_dict['y_train']).sum() / len(y_train_pred)
    y_val_pred = cls.predict(data_dict['X_val'])
    val_acc = (y_val_pred == data_dict['y_val']).sum() / len(y_val_pred)
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        ...
        scores_shifted = scores - scores_max  # prevent the score is too big and exp(score) is out of range
        scores_exp = torch.exp(scores_shifted)
        correct_class_score_exp = scores_exp[y[i]]
        scores_exp_tot = scores_exp.sum()
        loss -= torch.log(correct_class_score_exp / scores_exp_tot)
        for j in range(num_classes):
            if j == y[i]:
                dW[:, j] += (scores_exp[j] / scores_exp_tot - 1) * X[i]
            else:
                dW[:, j] += (scores_exp[j] / scores_exp_tot) * X[i]
...
def softmax_loss_vectorized(
    ...
    scores_exp = torch.exp(scores_shifted)
    correct_class_score_exp = scores_exp[range(num_train), y]
    scores_exp_tot = scores_exp.sum(dim=1)
    loss -= torch.log(correct_class_score_exp / scores_exp_tot).sum()
    dW += X.t().mm(scores_exp / scores_exp_tot.reshape(-1, 1))
    t2 = torch.zeros((num_train, num_classes), device=X.device, dtype=X.dtype)
    t2[range(num_train), y] = 1
    dW -= X.t().mm(t2)
    loss = loss / num_train + reg * torch.sum(W * W)
