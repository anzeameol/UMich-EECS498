...
def nn_forward_backward(
    ...
    scores_shifted = scores - scores_max.reshape(-1, 1)
    scores_exp = torch.exp(scores_shifted)
    correct_class_score_exp = scores_exp[range(N), y]
    scores_exp_tot = scores_exp.sum(dim=1)
    loss -= torch.log(correct_class_score_exp / scores_exp_tot).sum()
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    dS = scores_exp / scores_exp_tot.reshape(-1, 1)
    dS[range(N), y] -= 1
    dW2 = h1.t().mm(dS)
    dW2 = dW2 / N + 2 * reg * W2
    db2 = dS.sum(dim=0)
    db2 = db2 / N
    dh1 = dS.mm(W2.t())
    dh1_before_relu = dh1.clone()
    dh1_before_relu[h1 == 0] = 0
    db1 = dh1_before_relu.sum(dim=0)
    db1 = db1 / N
    dW1 = X.t().mm(dh1_before_relu)
    dW1 = dW1 / N + 2 * reg * W1
    grads['W2'] = dW2
    grads['b2'] = db2
    grads['W1'] = dW1
...
def nn_predict(
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    _, y_pred = ((X.mm(params['W1']) + params['b1']).mm(params['W2']) + params['b2']).max(dim=1)
    ###########################################################################
    #                              END OF YOUR CODE                           #
    ###########################################################################
...
def nn_get_search_params():
    ...
    learning_rate_decays = [1, 0.99, 0.95, 0.9]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def find_best_net(
    ...
    output_size = max(data_dict['y_train'].max(), data_dict['y_val'].max())
    for (lr, hs, reg, lrd) in itertools.product(learning_rates, hidden_sizes, regularization_strengths,
                                                learning_rate_decays):
        two_layer_net = TwoLayerNet(input_size=input_size, output_size=10, hidden_size=hs)
        history = two_layer_net.train(X=data_dict['X_train'], y=data_dict['y_train'], X_val=data_dict['X_val'],
                                      y_val=data_dict['y_val'], learning_rate=lr, learning_rate_decay=lrd, reg=reg, num_iters=1000, batch_size=200)
        accuracy = sum(history['val_acc_history']) / len(history['val_acc_history'])
        if accuracy > best_val_acc:
            best_val_acc = accuracy
            best_stat = history
            best_net = two_layer_net
    #############################################################################
    #                               END OF YOUR CODE                            #
    #############################################################################
