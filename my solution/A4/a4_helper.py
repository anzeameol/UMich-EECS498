...
class MyDataForInference(torch.utils.data.Dataset):
    voc_classes = [
        "aeroplane",
        "bicycle",
        "bird",
    ...
    idx_to_class = {_idx: _class for _idx, _class in enumerate(voc_classes)}
    ...
    def __getitem__(self, index: int):
        image_path = os.path.join(self.dataset_dir, self.names[index])
        image = Image.open(image_path).convert("RGB")
        ...
        return self.names[index], image

    def __len__(self):
        return len(self.names)


def get_data_loader(dataset_dir: str, image_size: int):
    dataset = MyDataForInference(dataset_dir, image_size)
...
def predict(
    ...
    for iter_num, test_batch in enumerate(test_loader):
        image_paths, images, gt_boxes = test_batch
        # print(f"iter:{iter_num}, Processing {image_paths[0]}")
        images = images.to(dtype=dtype, device=device)
        ...
        # Skip current iteration if no predictions were found.
        if pred_boxes.shape[0] == 0:
            print(f"{image_paths[0]}: No predictions found.")
            continue
        ...
        eecs598.utils.detection_visualizer(
            ...
            gt_boxes,
            pred_boxes,
            save_path=os.path.join(output_dir, image_paths[0]),
        )
