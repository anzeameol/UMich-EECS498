...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(self, num_classes: int, in_channels: int, stem_channels: List[int]):
        ...
        self.pred_box = nn.Conv2d(in_channel, 4, kernel_size=3, stride=1, padding=1)
        self.pred_ctr = nn.Conv2d(in_channel, 1, kernel_size=3, stride=1, padding=1)

        self.pred_cls.bias.data.zero_()
        self.pred_cls.weight.data.normal_(mean=0, std=0.01)

        self.pred_box.bias.data.zero_()
        self.pred_box.weight.data.normal_(mean=0, std=0.01)

        self.pred_ctr.bias.data.zero_()
        self.pred_ctr.weight.data.normal_(mean=0, std=0.01)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for key, feature in feats_per_fpn_level.items():
            # print(feature.shape)
            cls = self.stem_cls(feature)
            box = self.stem_box(feature)
            ctr = self.stem_box(feature)

            # print(cls.shape)
            cls = self.pred_cls(cls)
            B, C, H, W = cls.shape
            cls_logit = cls.reshape(B, C, H * W).permute(0, 2, 1)

            box = self.pred_box(box)
            B, C, H, W = box.shape
            box_logit = box.reshape(B, C, H * W).permute(0, 2, 1)

            ctr = self.pred_ctr(ctr)
            B, C, H, W = ctr.shape
            ctr_logit = ctr.reshape(B, C, H * W).permute(0, 2, 1)

            class_logits[key] = cls_logit
            boxreg_deltas[key] = box_logit
            centerness_logits[key] = ctr_logit
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def fcos_get_deltas_from_locations(
    ...
    # Replace "pass" statement with your code
    N = locations.shape[0]
    deltas = torch.zeros(size=(N, 4), dtype=gt_boxes.dtype, device=gt_boxes.device)
...
def fcos_apply_deltas_to_locations(
    ...
    # Replace "pass" statement with your code
    N = deltas.shape[0]
    output_boxes = torch.zeros(size=(N, 4), device=deltas.device, dtype=deltas.dtype)
    ...
    #         continue
    #     xc, yc = locations[i]
    #     left, top, right, bottom = deltas[i] * stride
    #     x1 = xc - left
    #     x2 = xc + right
    #     y1 = yc - top
    #     y2 = yc + bottom
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    centerness = (
        torch.min(deltas[:, 0], deltas[:, 2])
        / torch.max(deltas[:, 0], deltas[:, 2])
        * torch.min(deltas[:, 1], deltas[:, 3])
        / torch.max(deltas[:, 1], deltas[:, 3])
    ) ** 0.5
    ...
    #         centerness[i] = -1
    #         continue
    #     centerness[i] = (min(l, r) / max(l, r) * min(t, b) / max(t, b)) ** 0.5
    # print(centerness)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        # Replace "pass" statement with your code
        backbone_feat = self.backbone(images=images)
        ...
        shape_pre_fpn_level = {
            level_name: feat.shape for level_name, feat in backbone_feat.items()
        }
        ...
        for i in range(B):
            matched_gt_boxes.append(
                fcos_match_locations_to_gt(
                    locations_per_fpn_level, stride_per_fpn_level, gt_boxes[i]
                )
            )
        ...
        for i in range(B):
            ...
            for key in locations_per_fpn_level.keys():
                gt_deltas[key] = fcos_get_deltas_from_locations(
                    locations_per_fpn_level[key],
                    matched_gt_boxes[i][key],
                    stride_per_fpn_level[key],
                )
        ...
        # to dictionaries of batched tensors
        ...
        # 将p3，p4，p5的tensor聚合成一个tensor，这样可以批量计算，因为都是逐元素计算的
        # out: (B,H*W*3,...)
        ...
        # Replace "pass" statement with your code
        num_locations_across_fpn_levels = matched_gt_boxes.shape[1]

        gt_cls_logits = torch.zeros(
            size=(B * num_locations_across_fpn_levels, self.num_classes + 1),
            dtype=images.dtype,
            device=images.device,
        )
        # for i in range(B):
        #     for j in range(num_locations_across_fpn_levels):
        #         gt_cls_logits[i][j][matched_gt_boxes[i][j][4].long()] = 1
        gt_cls_logits[
            torch.arange(B * num_locations_across_fpn_levels),
            matched_gt_boxes[:, :, -1].reshape(-1).long(),
        ] = 1
        gt_cls_logits = gt_cls_logits[:, :-1].reshape(
            B, num_locations_across_fpn_levels, self.num_classes
        )
        loss_cls = sigmoid_focal_loss(inputs=pred_cls_logits, targets=gt_cls_logits)
        ...
        matched_gt_centerness = torch.zeros(
            size=(B, num_locations_across_fpn_levels),
            dtype=images.dtype,
            device=images.device,
        )
        matched_gt_centerness = fcos_make_centerness_targets(
            matched_gt_deltas.reshape(-1, 4)
        ).reshape(B, num_locations_across_fpn_levels, 1)
        # for i in range(B):
        #     matched_gt_centerness[i] = fcos_make_centerness_targets(
        #         matched_gt_deltas[i]
        #     )
        loss_ctr = F.binary_cross_entropy_with_logits(
            input=pred_ctr_logits,
            target=matched_gt_centerness,
            reduction="none",
        )
        loss_ctr[matched_gt_centerness < 0] *= 0
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            _, level_pred_classes = level_cls_logits.max(dim=1)
            pred_scores = level_pred_scores[torch.arange(N), level_pred_classes]

            # Step 2:
            # Replace "pass" statement with your code
            idx = pred_scores > test_score_thresh
            ...
            level_pred_scores = pred_scores[idx]
