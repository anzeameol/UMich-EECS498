...
def generate_token_dict(vocab):
    ...
    token_dict = {key: value for value, key in enumerate(vocab)}
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return token_dict
...
def prepocess_input_sequence(
    ...
    for token in tokens:
        ...
        elif token in spc_tokens:
            out.append(len(token_dict))
        else:
...
def scaled_dot_product_two_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    tmp = query.bmm(key.permute(0, 2, 1))
    tmp /= M**0.5
...
def scaled_dot_product_no_loop_batch(
    ...
    if mask is not None:
        ...
        tmp[mask] = -1e9
    # Replace "pass" statement with your code
    weights_softmax = F.softmax(tmp, dim=2)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        # Replace "pass" statement with your code
        N, K, M = query.shape
        outList = [attention(query, key, value, mask) for attention in self.heads]
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        self.MultiHeadBlock = MultiHeadAttention(
            num_heads=num_heads, dim_in=emb_dim, dim_out=emb_dim // num_heads
        )
        self.ln1 = LayerNormalization(emb_dim=emb_dim)
        self.ln2 = LayerNormalization(emb_dim=emb_dim)
    ...
    def forward(self, x):
        ...
        # Replace "pass" statement with your code
        """ 
        原论文的dropout是设置在残差连接和layer normalization之前的，这里设置在之后了 
        """
        residual1 = x
        out = self.MultiHeadBlock(x, x, x)
        out = out + residual1
        out = self.ln1(out)
...
def get_subsequent_mask(seq):
    ...
    N, K = seq.shape
    mask = torch.triu(
        torch.ones(size=(N, K, K), dtype=torch.bool, device=seq.device), diagonal=1
    )
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, dec_inp: Tensor, enc_inp: Tensor, mask: Tensor = None) -> Tensor:
        ...
        residual = out
        out = self.attention_cross(out, enc_inp, enc_inp)
        out = out + residual
        out = self.norm2(out)
        out = self.dropout(out)
        residual = out
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    y = torch.arange(0, 1, 1 / K, dtype=torch.float32).repeat(M, 1).t().unsqueeze(dim=0)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    pos = torch.arange(K).reshape(1, K, 1)
    dim = torch.arange(M).reshape(1, 1, M)
    # value = pos / (10000 ** (dim / M))
    value = pos / (10000 ** (torch.div(dim, M, rounding_mode="floor")))
    # 似乎有问题，如果按照ipynb里面的实现，那a永远是0了
    y = torch.where((dim % 2).bool(), torch.cos(value), torch.sin(value))
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        q_emb_oup = self.encoder(q_emb_inp)
        mask = get_subsequent_mask(ans_b[:, :-1])
        dec_out = self.decoder(a_emb_inp, q_emb_oup, mask)
