...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    y = x.mm(Wx) + prev_h.mm(Wh) + b
    next_h = torch.tanh(y)
    cache = (x, Wx, prev_h, Wh, y)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
    return next_h, cache
...
def rnn_step_backward(dnext_h, cache):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x, Wx, prev_h, Wh, y = cache
    # y = x * Wx + prev_h * Wh
    dy = (1 - torch.tanh(y) ** 2) * dnext_h
    db = dy.sum(dim=0)
    dx = dy.mm(Wx.t())
    dprev_h = dy.mm(Wh.t())
    dWx = x.t().mm(dy)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    y = []
    for i in range(T):
        tmp = x[:, i, :].mm(Wx) + hidden.mm(Wh) + b
        y.append(tmp.unsqueeze(dim=1))
        hidden = torch.tanh(tmp)
        h.append(hidden.unsqueeze(dim=1))
    h = torch.cat(h, dim=1)
    y = torch.cat(y, dim=1)
    cache = (h, h0, x, Wx, Wh, y)
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
    return h, cache
...
def rnn_backward(dh, cache):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    h, h0, x, Wx, Wh, y = cache
    N, T, D = x.shape
    H = dh.shape[-1]
    dx = torch.zeros_like(x, dtype=x.dtype, device=x.device)
    dWh = torch.zeros_like(Wh, dtype=Wh.dtype, device=Wh.device)
    db = torch.zeros(size=(H,), dtype=Wh.dtype, device=Wh.device)
    dWx = torch.zeros_like(Wx, dtype=Wx.dtype, device=Wx.device)
    dh_ = torch.zeros_like(h0, dtype=h0.dtype, device=h0.device)
    for i in range(T - 1, -1, -1):
        prev_h = h[:, i - 1, :] if i != 0 else h0
        dx_, dh_, dWx_, dWh_, db_ = rnn_step_backward(
            dh_ + dh[:, i, :], (x[:, i, :], Wx, prev_h, Wh, y[:, i, :])
        )
        dx[:, i, :] = dx_
        dWh += dWh_
        db += db_
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        if self.cell_type == "rnn":
            ...
            self.word_embedding = WordEmbedding(
                vocab_size=vocab_size, embed_size=wordvec_dim
            )
            self.output_proj = nn.Linear(
                in_features=hidden_dim, out_features=vocab_size
            )
            self.feature_proj = nn.Linear(
                in_features=input_dim * 16, out_features=hidden_dim
            )
            ...
            self.lstm_attn = AttentionLSTM(input_dim=wordvec_dim, hidden_dim=hidden_dim)
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def forward(self, images, captions):
        ...
        if self.cell_type == "rnn":
            ...
            h0 = self.feature_proj(feature.reshape(feature.shape[0], -1))
            emb_words = self.word_embedding(captions_in)
            out = self.rnn(emb_words, h0)
            ...
            h0 = self.feature_proj(feature.reshape(feature.shape[0], -1))
            emb_words = self.word_embedding(captions_in)
            out = self.lstm(emb_words, h0)
            scores = self.output_proj(out)
            loss = temporal_softmax_loss(scores, captions_out, ignore_index=self._null)
        elif self.cell_type == "attn":
            feature = self.image_encoder(images)
            a = self.feature_proj(feature.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
            emb_words = self.word_embedding(captions_in)
            out = self.lstm_attn(emb_words, a)
            scores = self.output_proj(out)
            loss = temporal_softmax_loss(scores, captions_out, ignore_index=self._null)
        ######################################################################
        #                           END OF YOUR CODE                         #
    ...
    def sample(self, images, max_length=15):
        ...
        if self.cell_type == "rnn":
            ...
            for i in range(max_length):
                # print(pre_word)
                emb_word = self.word_embedding(pre_word)
                out = self.rnn(emb_word.unsqueeze(dim=1), h0)
                scores = self.output_proj(out)
                _, idx = scores.max(dim=-1)
                pre_word = captions[:, i] = idx.squeeze(dim=1)
        elif self.cell_type == "lstm":
            feature = self.image_encoder(images)
            h0 = self.feature_proj(feature.reshape(feature.shape[0], -1))
            N = images.shape[0]
            ...
            for i in range(max_length):
                # print(pre_word)
                emb_word = self.word_embedding(pre_word)
                out = self.lstm(emb_word.unsqueeze(dim=1), h0)
                scores = self.output_proj(out)
                _, idx = scores.max(dim=-1)
                pre_word = captions[:, i] = idx.squeeze(dim=1)
        elif self.cell_type == "attn":
            ...
            for i in range(max_length):
                # print(pre_word)
                emb_word = self.word_embedding(pre_word)
                out = self.lstm_attn(emb_word.unsqueeze(dim=1), a)
                scores = self.output_proj(out)
                _, idx = scores.max(dim=-1)
                pre_word = captions[:, i] = idx.squeeze(dim=1)
        ######################################################################
...
class LSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, h0: torch.Tensor) -> torch.Tensor:
        ...
        hn = []
        for i in range(T):
            h, c = self.step_forward(x[:, i, :], prev_h=h, prev_c=c)
            hn.append(h.unsqueeze(dim=1))
...
def dot_product_attention(prev_h, A):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    attn_weights = prev_h.unsqueeze(dim=1).bmm(A.reshape(N, H, 16)).squeeze(dim=1) / (
        H**0.5
    )
    attn_weights = torch.softmax(attn_weights, dim=-1)
    attn = A.reshape(N, H, 16).bmm(attn_weights.unsqueeze(dim=-1)).squeeze(dim=-1)
...
class AttentionLSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, A: torch.Tensor):
        ...
        for i in range(T):
            ...
            h, c = self.step_forward(x[:, i, :], prev_h=h, prev_c=c, attn=attn)
            hn.append(h.unsqueeze(dim=1))
        hn = torch.cat(hn, dim=1)
        ######################################################################
        #                           END OF YOUR CODE                         #
