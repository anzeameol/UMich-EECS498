...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    #         img1 = x_train[i].view(-1)
    #         img2 = x_test[j].view(-1)
    #         dis = 0
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
      ...
      _, y_pred[i] = torch.max(torch.bincount(y_train[idx]),dim=0)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    for k in k_choices:
      ...
      for i in range(num_folds):
        xTrain = torch.cat([x_train_folds[j] for j in range(num_folds) if j != i])
        yTrain = torch.cat([y_train_folds[j] for j in range(num_folds) if j != i])
        knnClassifier = KnnClassifier(xTrain, yTrain)
        accuracy.append(knnClassifier.check_accuracy(x_train_folds[i], y_train_folds[i], k))
